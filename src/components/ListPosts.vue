...
<template>
  <ul>
    ...
    <template v-for="route, idx in posts" :key="route.path">
      ...
      >
        <span text-8em color-transparent absolute left--3rem top--2rem font-bold text-stroke-2 text-stroke-hex-aaa op15>{{ getGroupName(route) }}</span>
      </div>
      <div
        class="slide-enter"
        :style="{
          ...
          <li class="no-underline" flex="~ col md:row gap-2 md:items-center">
            <div class="title text-lg leading-1.2em" flex="~ gap-2 wrap">
              <span
                ...
                class="text-xs bg-zinc:15 text-zinc5 rounded px-1 py-0.5 md:ml--12 mr2 my-auto hidden md:block"
              >中文</span>
